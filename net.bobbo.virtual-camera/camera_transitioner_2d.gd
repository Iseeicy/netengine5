## A node that transitions an `EntityCamera2D` from some starting point
## to a `VirtualCamera2D`, given a `CameraTransition` resource to use
## as config. Not meant to be created manually.
class_name CameraTransitioner2D
extends Node
...
var target_camera: EntityCamera2D

## The virtual camera to end up at when completing the transition.
var destination_vcam: VirtualCamera2D
...
func _init(
	...
	target_camera: EntityCamera2D,
	destination_vcam: VirtualCamera2D
):
...
func _calculate_tweened_position() -> Vector2:
...
func _calculate_tweened_rotation() -> float:
	return Tween.interpolate_value(
		_starting_rotation,
