...
func character_agent_physics_process(_delta: float) -> void:
	# Try to pickup any cached items that couldn't be picked up before,
	# but MAYBE could be now
...
func _on_item_pickup_area_3d_body_entered(body: Node3D):
	...
	if not (body is WorldItem3D):
		return

	# Try to pick up the item
	var could_pickup = _attempt_pickup_item(body)

	# If we couldn't pick up the item NOW, cache the item in a list to
	# be re-checked to see if we CAN pick it up later.
	if not could_pickup:
		_items_that_could_be_picked_up.append(body)
