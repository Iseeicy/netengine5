...
func state_enter(_message: Dictionary = {}) -> void:
	...
	text_window.request_choice_hover.connect(
		_on_text_window_request_choice_hover.bind()
	)
	text_window.request_choice_confirm.connect(
		_on_text_window_request_choice_confirm.bind()
	)
	...
	for index in range(0, choice_data.options.size()):
		...
		if (
			choice_data
			. options[index][ChoicePromptNodeData.OPTIONS_VIS_COND_KEY]
			. get_value()
		):
			_option_index_translation[visible_options.size()] = index
			visible_options.push_back(choice_data.options[index])
...
func state_exit() -> void:
	text_window.choice_confirmed.disconnect(
		_on_text_window_choice_confirm.bind()
	)
	text_window.request_choice_confirm.disconnect(
		_on_text_window_request_choice_confirm.bind()
	)
	text_window.request_choice_hover.disconnect(
		_on_text_window_request_choice_hover.bind()
	)
...
func _options_to_choice_prompt(
	text: String, options: Array[Dictionary]
) -> TextWindowChoicePrompt:
...
func _on_text_window_choice_confirm(
	visible_index: int, _prompt: TextWindowChoicePrompt
) -> void:
